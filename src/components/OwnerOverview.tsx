  ...
  return (
    <div id="owner-overview-canvas" className="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-outline-variant">
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold text-on-surface">Owner Overview</h1>
          <p className="text-sm text-on-surface-variant mt-1">Portfolio performance across all locations.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <select
              ...
              className="h-11 px-4 pr-8 rounded-lg border border-outline-variant bg-surface-container-lowest text-xs font-semibold text-on-surface shadow-2xs appearance-none cursor-pointer focus:outline-none focus:border-primary"
            >
          ...
          <button
            ...
            className="h-11 px-4 rounded-lg bg-primary text-on-primary text-xs font-semibold hover:bg-primary-hover transition-colors flex items-center gap-2 shadow-sm cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
        <div
          id="kpi-card-revenue"
          className="bg-surface-container-lowest rounded-xl p-6 border border-outline-variant shadow-[0_4px_12px_rgba(0,0,0,0.02)] transition-transform hover:-translate-y-0.5"
        >
          ...
          <p className="mt-2 text-xs text-outline">Across {properties.length} propert{properties.length === 1 ? 'y' : 'ies'}</p>
        </div>
        ...
        >
          ...
          <p className="mt-2 text-xs text-outline">Room service, since midnight</p>
        </div>
        ...
        >
          ...
          <p className="mt-2 text-xs text-outline">All-time, all properties</p>
        </div>
        ...
        >
          ...
          <p className="mt-2 text-xs text-outline">Unresolved guest requests</p>
        </div>
      </div>
      ...
      {ticketAnalytics && ticketAnalytics.ordersTotal > 0 && (
        <div
          ...
          className="bg-highlight rounded-xl p-6 border border-warning flex flex-col sm:flex-row sm:items-center gap-6 justify-between"
        >
          <div>
          ...
          <div className="flex items-center gap-8">
            <div>
              ...
              <div className="text-[11px] text-on-surface-variant">Avg order · with recommendation</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-on-surface">
                {formatCurrency(ticketAnalytics.avgOrderValueWithoutRecommendation, primaryCurrency)}
              </div>
              <div className="text-[11px] text-on-surface-variant">Avg order · without</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div 
            ...
            className="bg-surface-container-lowest rounded-xl p-6 border border-outline-variant shadow-[0_4px_12px_rgba(0,0,0,0.02)]"
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
              <div>
                <h3 className="text-lg font-semibold text-on-surface">Revenue Trends</h3>
                <p className="text-xs text-on-surface-variant">Gross operating revenue daily breakdown</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex bg-surface-container-low p-1 rounded-lg border border-outline-variant text-xs">
                  <button
                    ...
                    className={`px-3 py-1 rounded font-medium transition-colors ${
                      activeChartMetric === 'revenue' ? 'bg-surface-container-lowest text-primary shadow-xs font-semibold' : 'text-on-surface-variant'
                    }`}
                  >
                    Revenue
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded font-medium transition-colors ${
                      activeChartMetric === 'orders' ? 'bg-surface-container-lowest text-primary shadow-xs font-semibold' : 'text-on-surface-variant'
                    }`}
                  >
                    Orders
                  </button>
                ...
                <button 
                  ...
                  className="text-xs text-primary hover:underline font-semibold uppercase tracking-wider shrink-0 cursor-pointer ml-2"
                >
                  View Report
                </button>
              </div>
          ...
          <div 
            ...
            className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_4px_12px_rgba(0,0,0,0.02)] overflow-hidden"
          >
            <div className="p-6 border-b border-outline-variant flex justify-between items-center">
              <div>
                <h3 className="text-lg font-semibold text-on-surface">Property Performance</h3>
                <p className="text-xs text-on-surface-variant">Comparative operational yield across luxury units</p>
              </div>
              <button 
                onClick={() => onNavigate('properties')}
                  ...
                  {properties.map((prop) => (
                    ...
                    >
                      <td className="p-4 flex items-center gap-3">
                        <div className="w-10 h-10 rounded-md bg-outline-variant overflow-hidden shrink-0 border border-outline-variant">
                          <img 
                        ...
                        <div>
                          <span className="font-semibold group-hover:text-primary transition-colors block">
                            {prop.name}
                          </span>
                          <span className="text-xs text-outline">{prop.location}</span>
                        </div>
                      </td>
                      ...
                      <td className="p-4 text-right">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${
                          prop.occupancy >= 85 ? 'bg-success/10 text-success' : 'bg-warning/20 text-primary'
                        }`}>
                          {prop.occupancy}%
                        </span>
                      </td>
        ...
        <div className="space-y-6">
          <div
            ...
            className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_4px_12px_rgba(0,0,0,0.02)] p-6"
          >
            <div className="flex items-center gap-2 mb-6 pb-2 border-b border-outline-variant">
              <AlertTriangle className="w-5 h-5 text-warning" />
              <h3 className="text-lg font-semibold text-on-surface">Attention Required</h3>
              {(() => {
            ...
            <div className="space-y-4">
              {properties.filter(p => p.requestsCount > 0).length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-6 text-center">
                  ...
                  <p className="text-xs font-semibold text-on-surface-variant">All caught up — no open requests.</p>
                </div>
              ) : (
                properties.filter(p => p.requestsCount > 0).map(p => (
                  <div
                    ...
                    className="p-4 rounded-lg bg-error-container/20 border border-error-container"
                  >
                    <div className="flex gap-3">
                      <Megaphone className="w-5 h-5 text-error shrink-0 mt-0.5" />
                      <div>
                        <h4 className="text-xs font-bold text-on-surface uppercase tracking-wider">{p.name}</h4>
                        ...
                        <button
                          ...
                          className="mt-2 text-xs font-semibold text-primary uppercase tracking-wider hover:underline cursor-pointer flex items-center gap-1"
                        >
          ...
          <div className="bg-surface-container-lowest rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-outline-variant">
            <div className="flex items-center justify-between pb-4 border-b border-outline-variant">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-lg text-on-surface">Portfolio Report</h3>
              </div>
              <button onClick={() => setShowReportModal(false)} className="text-gray-400 hover:text-gray-600 text-xl font-bold">
                &times;
              </button>
            </div>
            <div className="py-4 space-y-3 text-xs text-on-surface-variant">
              ...
              <div className="bg-surface-container-low p-3 rounded-lg border border-outline-variant space-y-2 font-mono">
                {properties.map(p => (
                  <div key={p.id} className="flex justify-between">
                    <span>{p.name}:</span>
                    <span className="font-bold text-on-surface">{p.totalRevenue} · {p.occupancy}% occupied</span>
                  </div>
                ))}
                <div className="border-t border-outline-variant pt-1 flex justify-between font-bold text-sm text-primary">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 border border-outline-variant rounded-lg text-xs font-semibold text-on-surface-variant hover:border-primary hover:text-primary flex items-center gap-1.5"
              >
